...
class BackupController extends Controller
{
	...
	public function actionDown($id)
	{
	  ...
	  $file = $list[$id];
		if ( isset($file))
		{
			...
			if ( file_exists($sqlFile))
			{
				Yii::$app->response->sendFile($sqlFile)->send();
			}
	...
	protected function updateMenuItems($model = null)
	{
		...
		switch( $this->action->id)
		{
			case 'restore':
				{
					$this->menu[] = array('label'=>Yii::t('app', '查看网站') , 'url'=>Yii::$app->HomeUrl);
				}
			case 'create':
				{
					$this->menu[] = array('label'=>Yii::t('app', '备份列表') , 'url'=>array('index'));
				}
				break;
			case 'upload':
				{
					$this->menu[] = array('label'=>Yii::t('app', '创建备份') , 'url'=>array('create'));
				}
				break;
			default:
				{
